...
class DerotateAndStackModule(ProcessingModule):
    ...
    def run(self) -> None:
        ...
        if self.m_stack is not None:
            attt_in = self.m_image_in_port.get_all_non_static_attributes()
            self.m_image_out_port.copy_attributes(self.m_image_in_port)
            for at in attt_in:    
                self.m_image_out_port.del_attribute(at)
                da = self.m_image_in_port.get_attribute(at)[0]
                self.m_image_out_port.add_attribute(at, np.array([da]), static = False)
                
        elif self.m_derotate is not None:
            self.m_image_out_port.copy_attributes(self.m_image_in_port)
            ...
            for key in non_static_attr:
                ...
                if key != 'INDEX' or (key == 'INDEX' and not self.m_index_init):
                    ...
                    if self.m_check_attr:
                        if key in ('PARANG', 'STAR_POSITION', 'INDEX', 'NFRAMES', 'LAMBDA', 'TIME'):
                            if status == 1:
